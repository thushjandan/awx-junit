...
def test_generate_report(awx_sample_output_single_entry, capsys):
    awx_junit.main(awx_sample_output_single_entry)
    captured = capsys.readouterr()
    tree = ET.fromstring(captured.out)
    assert tree.tag == 'testsuites'

def test_xml_output_single_entry(awx_sample_output_single_entry, capsys):
    awx_junit.main(awx_sample_output_single_entry)
    captured = capsys.readouterr()
    tree = ET.fromstring(captured.out)
    assert tree.tag == 'testsuites'
...
def test_xml_output_multiple_entries(awx_sample_output, capsys):
    ...
    assert tree.find("./testsuite[@name='Gathering Facts']").attrib['time'] == '7.417543'
...
def test_not_json(awx_sample_output, capsys):
    with pytest.raises(SystemExit) as pytest_wrapped_e:
        awx_junit.main(io.StringIO("foobar"))
    captured = capsys.readouterr()
    assert captured.err.startswith('Input is not a JSON') == True
